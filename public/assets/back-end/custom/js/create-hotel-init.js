...
function addRoomInfo() {
    ...
    room_category.forEach(element => {
        html += '<option value="' + element.id + '">' + element.room_category + '</option>'
    });
    ...
    room_type.forEach(element => {
        html += '<option value="' + element.id + '">' + element.room_type + '</option>'
    });
    ...
    $("#add-hotel-form").validate().settings.rules['room_category[]'] = {
        required: true
    };

    for (let i = 0; i < index; i++) {
        $("#add-hotel-form").validate().settings.rules['room_type[' + i + '][]'] = {
            required: true
        };
    }
